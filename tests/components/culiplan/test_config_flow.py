...
from custom_components.culiplan.config_flow import (
    CuliplanOptionsFlow,
    OAuth2FlowHandler,
)
...
async def test_options_flow_form_and_save(
    ...
    """Options form renders and saves."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id="user-42",
        data={"token": {"access_token": "x"}},
    ...
    with patch("custom_components.culiplan.async_setup_entry", return_value=True):
        ...
        result = await hass.config_entries.options.async_configure(
            result["flow_id"],
            user_input={CONF_EXPIRY_DAYS: 7, CONF_EXPIRY_HOURS: 24},
        )
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"] == {CONF_EXPIRY_DAYS: 7, CONF_EXPIRY_HOURS: 24}
...
async def test_handler_logger() -> None:
    ...
    handler = OAuth2FlowHandler()
    assert handler.logger.name == "custom_components.culiplan.config_flow"


async def test_async_get_options_flow() -> None:
    """``async_get_options_flow`` returns our options flow."""
    entry = MockConfigEntry(domain=DOMAIN, unique_id="u")
    assert isinstance(
        OAuth2FlowHandler.async_get_options_flow(entry), CuliplanOptionsFlow
    )
